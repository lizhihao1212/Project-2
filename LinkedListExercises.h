...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  Node *thru= head_;
  Node *thru_1 = other.head_;
    while(thru !=nullptr||thru_1!=nullptr){
      if(thru!=nullptr&&thru_1!=nullptr){
        if(thru->data<=thru_1->data){
        merged.pushBack(thru->data);
        left.popBack();
        thru = thru->next;
      }
      else{
        merged.pushBack(thru_1->data);
        right.popBack();
        thru_1 = thru_1->next;
        }}
        
      if(thru==nullptr){
        while(thru_1!=nullptr){
          merged.pushBack(thru_1->data);
          thru_1 = thru_1->next;
